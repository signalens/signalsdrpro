...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    while (not stop_signal_called) {
        ...
        output_reg_values(gpio_bank, port, usrp, num_bits, has_src_api);
         ...
         usrp->set_gpio_attr(gpio_bank, "DDR", 0xffff,0xffff);
         usrp->set_gpio_attr(gpio_bank, "CTRL", 0, 0xffff);
        while(1){
         usrp->set_gpio_attr(gpio_bank, "OUT", 0xffff,0xffff);// GPIO_BIT(4));
        sleep(1);
         usrp->set_gpio_attr(gpio_bank, "OUT", 0,0xffff);// GPIO_BIT(4));
        sleep(1);
        }
